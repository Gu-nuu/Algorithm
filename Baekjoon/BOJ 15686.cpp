...
void solve(int index, int count)        //브루트포스 알고리즘을 사용하기 때문에 index 값을 파라미터로 설정한다.
{
    ...
    //브루트포스 알고리즘을 통해 index 값을 증가시켜 탐색하다 보면 치킨집의 갯수를 넘어서서 탐색하는 경우가 발생하므로 이는 예외 처리해준다.
    if (index == chicken.size())
        return;

    isVisited[index] = true;
    solve(index + 1, count + 1);
    //만약 여러 개의 치킨 집 중 내가 고르고자 하는 치킨집이라면 true로 설정 후 index와 count를 둘 다 증가시켜준다.

    isVisited[index] = false;
    solve(index + 1, count);
    //내가 고르고자 하는 치킨집이 아니라면 false로 설정 후 index 값만 증가시켜 다른 치킨집을 탐색한다.
